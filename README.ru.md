...
`exportOrFail()` бросает на **любом** неудачном сообщении, включая
запланированный ретрай, — временная авария ClickHouse его тоже поднимает. На
`skipped` он не бросает: эти сообщения ещё выжидают свой backoff, попытки по ним
не было, поэтому они остаются неопубликованными, но неудачей не считаются. Если
будить дежурного нужно только на невосстановимых сообщениях, проверяйте
`ClickHouseExportResult::hasTerminalFailures()`.

### Сообщения в backoff не захватываются

Если внедрённое хранилище реализует `Rasuvaeff\Yii3Outbox\RetryAwareStorageInterface`
(а `rasuvaeff/yii3-outbox-db` 2.2.0 реализует), экспортёр захватывает через
`claimReady()`, и сообщение, у которого не истекла задержка повтора, из таблицы
вообще не забирается. Раньше захватывались любые `Pending`-строки обслуживаемых
типов, а неготовые тут же писались обратно: две записи на каждое отложенное
сообщение за прогон, и каждое занимало слот в `fetchLimit`, который мог достаться
готовому, — поэтому задержка экспорта росла вместе с очередью повторов.

Хранилище без этого интерфейса продолжает работать: экспортёр откатывается на
`claim()` и фильтрует в PHP, ровно как раньше.

Следствие, о котором стоит знать до настройки алертов: `ClickHouseExportResult::$skipped`
с retry-aware хранилищем всегда `0`, потому что сообщения, которые он считал,
больше не захватываются. Это никогда не была глубина очереди — это была
